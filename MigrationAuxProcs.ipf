	...
	for(i = 0; i < cond; i += 2)
		...
		for(j = 0; j < nWaves; j += 1)
			...
			if(WaveExists(w1) == 1)
				...
				Duplicate/O/FREE w1 w1s
				newName = "c_" + ReplaceString("pre",wName0,"_")
				Concatenate/O/NP {w0s,w1s}, tempwave
				Make/O/N=(floor(numpnts(tempwave)/3)) $newName
				Wave w2 = $newName
				SetScale/P x 0, 60, w2
				// assume delta = 20. Hourly average
				w2 = mean(tempwave, x, x+2*deltax(tempwave))
